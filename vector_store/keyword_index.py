...
class KeywordIndex:
    ...
    def add_document(
        self, doc_id: str, content: str, keywords: List[str], metadata: Dict = None
    ):
        """添加文档到关键词索引"""
        conn = sqlite3.connect(self.db_path)
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?)
        """,
            (doc_id, content, " ".join(keywords), json.dumps(metadata or {})),
        )
    ...
    def _update_keyword_stats(
        self, conn: sqlite3.Connection, doc_id: str, content: str, keywords: List[str]
    ):
        """更新关键词统计信息"""
        # 计算词频
    ...
    def search_with_bm25(
        self, query: str, k1: float = 1.2, b: float = 0.75, limit: int = 100
        ...
        for term in query_terms:
            ...
            for row in cursor.fetchall():
                ...
                score = self._calculate_bm25_score(
                    tf,
                    self._get_doc_length(conn, doc_id),
                    avg_doc_len,
                    self._get_df(conn, term),
                    k1,
                    b,
                )
    ...
    def _get_total_docs(self, conn: sqlite3.Connection) -> int:
        """获取总文档数"""
        cursor = conn.execute(
            "SELECT COUNT(DISTINCT doc_id) as count FROM doc_keywords"
        )
    ...
    def _get_avg_doc_length(self, conn: sqlite3.Connection) -> float:
        """获取平均文档长度"""
        cursor = conn.execute(
            "SELECT AVG(LENGTH(content)) as avg_len FROM keyword_index"
        )
        result = cursor.fetchone()
        return result["avg_len"] or 100.0

    def _calculate_bm25_score(
        self, tf: float, doc_len: int, avg_doc_len: float, df: int, k1: float, b: float
    ) -> float:
        """计算BM25分数"""
