...
class EmbeddingUtil:
    ...
    def _get_embedding_provider_for_collection(self, user_prefs_handler, collection_name: str):
        ...
        astrbot_embedding_provider_id = (
            user_prefs_handler.user_collection_preferences.get("collection_metadata", {})
            .get(collection_name, {})
        ...
        if astrbot_embedding_provider_id:
            ...
            if provider:
                ...
                if isinstance(provider, EmbeddingProvider):
                    ...
                    logger.warning(f"提供商 {astrbot_embedding_provider_id} 不是 EmbeddingProvider 类型")
            ...
            if providers:
                ...
                for provider in providers:
                    if isinstance(provider, EmbeddingProvider):
                        ...
                        # 获取提供商名称
                        try:
                            provider_name = provider.meta().id
                        except:
                            provider_name = provider.provider_config.get('id', 'unknown')
                        logger.info(f"EmbeddingUtil: 使用默认embedding提供商 {provider_name}")
                        break
                
                if not self._default_provider:
                    logger.error("EmbeddingUtil: 没有找到可用的embedding提供商")
                    return None
            else:
    ...
    async def get_embedding_async(self, text: str, collection_name: str = None, user_prefs_handler = None) -> Optional[List[float]]:
        """获取单个文本的 embedding"""
        ...
        if collection_name and user_prefs_handler:
            provider = self._get_embedding_provider_for_collection(user_prefs_handler, collection_name)
        else:
        ...
        try:
            ...
            if hasattr(provider, 'get_embeddings'):
                ...
                if len(batch_texts) <= max_batch_size:
                    ...
                    # - 添加进度回调支持
                    logger.info(f"文本数量 {len(batch_texts)} 超过批次限制 {max_batch_size}，将分批处理")
                    
                    all_embeddings = []
                    for i in range(0, len(batch_texts), max_batch_size):
                        batch_chunk = batch_texts[i:i + max_batch_size]
                        batch_num = i//max_batch_size + 1
                        total_batches = (len(batch_texts) + max_batch_size - 1) // max_batch_size
                        logger.debug(f"处理批次 {batch_num}/{total_batches}: {len(batch_chunk)} 个文本")
                        
                        try:
                            chunk_embeddings = await provider.get_embeddings(batch_chunk)
                            if chunk_embeddings:
                                all_embeddings.extend(chunk_embeddings)
                            else:
                                # 如果批次失败，用None填充
                                all_embeddings.extend([None] * len(batch_chunk))
                                logger.warning(f"批次 {batch_num}/{total_batches} 处理失败：返回空结果")
                        except Exception as e:
                            # TODO: 添加更细粒度的错误处理
                            # - 区分不同类型的API错误（限流、配额、网络等）
                            # - 实现指数退避重试策略
                            # - 支持部分批次失败时的恢复机制
                            logger.error(f"批次 {batch_num}/{total_batches} 处理失败: {e}")
                            all_embeddings.extend([None] * len(batch_chunk))
                    ...
                    if len(all_embeddings) == len(batch_texts):
                        ...
                        logger.error(f"嵌入结果数量不匹配: 期望 {len(batch_texts)}, 实际 {len(all_embeddings)}")
            else:
                # 逐个处理
    ...
    def get_dimensions(self, collection_name: str = None, user_prefs_handler = None) -> Optional[int]:
        """获取 embedding 维度"""
        # 获取对应的提供商
        if collection_name and user_prefs_handler:
            provider = self._get_embedding_provider_for_collection(user_prefs_handler, collection_name)
